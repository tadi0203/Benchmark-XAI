# interpretability/new_bdd_XAI/RF_BDD_NEW_EXP.py
# Explicabilité : SHAP, PFI, PDP, LIME sur RF entrainée sur la nouvelle bdd
# ============================
#  BIBLIOTHÈQUES
# ============================
import os
...
EXPLAIN_BASE_PATH      = ROBUST / "Fidelite\XAI\Analysis\ rf"
...
def model_path_for_target(target: str) -> str:
    # Nom des fichiers modèles RF
    return os.path.join(MODEL_BASE_PATH, f"rf_{target}.joblib")
...
def save_shap_summary(model, X_bg: pd.DataFrame, X_test_sub: pd.DataFrame, name: str, save_dir: str):
    ...
    if isinstance(model, Pipeline):
        ...
        steps = model.steps
        if len(steps) == 1 and isinstance(steps[-1][1], RandomForestRegressor):
            rf_estimator = steps[-1][1]
            use_tree_explainer = True
    elif isinstance(model, RandomForestRegressor):
        rf_estimator = model
        use_tree_explainer = True

    if use_tree_explainer and rf_estimator is not None:
        # TreeExplainer directement sur l'estimateur
        explainer = shap.TreeExplainer(rf_estimator)
        # Prédire avec l'estimateur (pas la pipeline) si on a RF nu
...
def save_pfi(model, X_test_sub: pd.DataFrame, y_test_sub: pd.Series, name: str, save_dir: str):
    ...
    plt.title("Permutation Feature Importance (RF) — test subset (n=10)")
    out = os.path.join(save_dir, f"{name}.png")
    plt.tight_layout()
    plt.savefig(out, dpi=200)
    plt.close()
...
# ============================
#  Appliquer toutes les méthodes XAI (RF pipeline)
# ============================
def explain_rf_pipeline(path_model: str, X_bg: pd.DataFrame, X_test_sub: pd.DataFrame,
                        y_test_sub: pd.Series, target: str):
    """
    Charge la pipeline RF et applique SHAP (bg=100/test=10), PFI (test=10),
...
def main():
    ...
    if os.path.exists(path_N):
        explain_rf_pipeline(path_N, XN_bg, XN_test_sub, yN_test_sub, "N_sim")
    else:
        print(f"[!] Modèle RF introuvable pour N_sim : {path_N}")
    ...
    if os.path.exists(path_S):
        explain_rf_pipeline(path_S, XS_bg, XS_test_sub, yS_test_sub, "S_sim")
    else:
        print(f"[!] Modèle RF introuvable pour S_sim : {path_S}")
