# interpretability/new_bdd_XAI/SVR_BDD_NEW_EXP.py
# Explicabilité : SHAP, PFI, PDP, LIME sur SVR entrainée sur la nouvelle bdd
# ============================
#  BIBLIOTHÈQUES
# ============================
import os
...
#  Chemins E/S 
# ----------------------------
MODEL_BASE_PATH = ROBUST / "Fidelite\ML_models"
NEW_DATA_CSV = DATA / "perturbed_data.csv"  
EXPLAIN_BASE_PATH      = ROBUST / "Fidelite\XAI\Analysis\svr"
ensure_dir(EXPLAIN_BASE_PATH)
FEATURES+=["aux_1","aux_2"]
...
BG_SIZE = 40       # nb. de points train pour “entraîner” les explaineurs
...
def model_path_for_target(target: str) -> str:
    return os.path.join(MODEL_BASE_PATH, f"svm_{target}.joblib")
...
def save_pfi(model, X_test_sub: pd.DataFrame, y_test_sub: pd.Series, name: str, save_dir: str):
    """
    Importance des variables par permutation (PFI) calculée sur les 10 points de test.
    """
    ...
    plt.title("Permutation Feature Importance (SVR) — test subset (n=10)")
    out = os.path.join(save_dir, f"{name}.png")
    plt.tight_layout()
    plt.savefig(out, dpi=200)
    plt.close()
...
def save_pdp(model, X_bg: pd.DataFrame, features: list[str], name: str, save_dir: str):
    """
    PDP pour toutes les features (1D) en se basant sur les 100 points de train (background).
    """
...
def save_lime(model, X_bg: pd.DataFrame, X_test_sub: pd.DataFrame, name: str, save_dir: str):
    """
    LIME :
    - training_data = 100 points de train (X_bg)
    - explications sur 10 points de test (X_test_sub)
    - Sauvegarde CSV + barh + beeswarm
    """
    ...
    # Importance globale (|poids| moyen)
...
# ============================
#  Appliquer toutes les méthodes XAI (SVR pipeline)
# ============================
def explain_svr_pipeline(path_model: str, X_bg: pd.DataFrame, X_test_sub: pd.DataFrame,
                         y_test_sub: pd.Series, target: str):
    """
    Charge la pipeline SVR et applique SHAP (bg=100/test=10), PFI (test=10),
...
def main():
    ...
    if os.path.exists(path_N):
        explain_svr_pipeline(path_N, XN_bg, XN_test_sub, yN_test_sub, "N_sim")
    else:
        print(f"[!] Modèle SVR introuvable pour N_sim : {path_N}")
    ...
    if os.path.exists(path_S):
        explain_svr_pipeline(path_S, XS_bg, XS_test_sub, yS_test_sub, "S_sim")
    else:
        print(f"[!] Modèle SVR introuvable pour S_sim : {path_S}")
