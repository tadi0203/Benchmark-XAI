# interpretability/new_bdd_XAI/REG_Poly_BDD_NEW_EXP.py
# Explicabilité : SHAP, PFI, PDP, LIME sur Regression polynomiale entrainée sur la nouvelle bdd
# ============================
#  BIBLIOTHÈQUES
# ============================
import os
...
EXPLAIN_BASE_PATH      = ROBUST / "Fidelite\XAI\Analysis\ reg_poly"
...
def model_path_for_target(target: str) -> str:
    # Nom des fichiers modèles Poly
    return os.path.join(MODEL_BASE_PATH, f"polyreg_{target}_deg3.joblib")
...
def save_shap_summary(model, X_bg: pd.DataFrame, X_test_sub: pd.DataFrame, name: str, save_dir: str):
    """
    SHAP pour régression polynomiale (Pipeline PolyFeatures + LinearRegression typiquement) :
    KernelExplainer avec wrapper ndarray -> pipeline.predict.
    """
...
def save_pfi(model, X_test_sub: pd.DataFrame, y_test_sub: pd.Series, name: str, save_dir: str):
    ...
    plt.title("Permutation Feature Importance (Poly) — test subset (n=10)")
    out = os.path.join(save_dir, f"{name}.png")
    plt.tight_layout()
    plt.savefig(out, dpi=200)
    plt.close()
...
def save_pdp(model, X_bg: pd.DataFrame, features: list[str], name: str, save_dir: str):
    """
    PDP sur les 100 points de train. La Pipeline applique automatiquement
    PolynomialFeatures avant LinearRegression lors de predict().
    """
...
# ============================
#  Appliquer toutes les méthodes XAI (Poly pipeline)
# ============================
def explain_poly_pipeline(path_model: str, X_bg: pd.DataFrame, X_test_sub: pd.DataFrame,
                          y_test_sub: pd.Series, target: str):
    """
    Charge la pipeline Poly et applique SHAP (bg=100/test=10), PFI (test=10),
...
def main():
    ...
    if os.path.exists(path_N):
        explain_poly_pipeline(path_N, XN_bg, XN_test_sub, yN_test_sub, "N_sim")
    else:
        print(f"[!] Modèle Poly introuvable pour N_sim : {path_N}")
    ...
    if os.path.exists(path_S):
        explain_poly_pipeline(path_S, XS_bg, XS_test_sub, yS_test_sub, "S_sim")
    else:
        print(f"[!] Modèle Poly introuvable pour S_sim : {path_S}")
